...
export default function FrenshChallenge(props) {
    ...
    useEffect(() => {
        authContext.user.isFrValid ? setDisabled(true) : setDisabled(false)
    }, [authContext.user.isFrValid])
    ...
    const handleSubmit = async (responses) => {
        const result = await checkQuestions({ candidatAnswers: responses, candidatID: authContext.user._id, challengeType: "fr" })
    ...
    if (isLoading) return <Loading />
        ...
        return (
            <SafeAreaView style={styles.container} >
                <AppText style={styles.title}>Francais Test</AppText>
                <FlatList
                    data={questions}
