...
export default function EnglishChallenge(props) {
    ...
    useEffect(() => {
        authContext.user.isEngValid ? setDisabled(true) : setDisabled(false)
    }, [authContext.user.isEngValid])
    ...
    const handleSubmit = async (responses) => {
        const result = await checkQuestions({ candidatAnswers: responses, candidatID: authContext.user._id, challengeType: "eng" })
    ...
    if (isLoading) return <Loading />
        ...
        return (
            <SafeAreaView style={styles.container} >
                <AppText style={styles.title}>Anglais Test</AppText>
                <FlatList
                    data={questions}
